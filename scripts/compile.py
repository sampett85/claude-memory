...
Enhanced: adds project + tech stack auto-tagging to compiled articles
and triggers Obsidian sync after successful compilation.
...
from config import (
    AGENTS_FILE, CONCEPTS_DIR, CONNECTIONS_DIR, DAILY_DIR,
    DIGESTS_DIR, KNOWLEDGE_DIR, get_project_name, now_iso,
)
from utils import (
    file_hash, list_raw_files, list_wiki_articles,
    load_state, read_wiki_index, save_state,
)
...
Every article MUST include a `tags:` list in its YAML frontmatter:
- Always include the project tag: `{project}`
- Detect tech stack from content and add tags like: react, nextjs, supabase, typescript, tailwind, python, docker, etc.
- Detect domain from content and add tags like: auth, database, api, frontend, deployment, testing, performance, etc.
- Example: `tags: [{project}, react, supabase, auth]`
...
### Quality standards:
- Every article must have complete YAML frontmatter (including tags)
...
def main():
    ...
    parser.add_argument("--no-sync", action="store_true", help="Skip Obsidian sync after compile")
