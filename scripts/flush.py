...
Reads conversation context, uses Claude Agent SDK to decide what's worth
saving, and appends the result to today's daily log.

Enhanced: adds project name to every daily log entry for project-scoped memory.
...
_project_name = ROOT.parent.name
if _env_file.exists():
...
_compile_after_hour = 18
if _env_file.exists():
    for line in _env_file.read_text().splitlines():
        if line.startswith("COMPILE_AFTER_HOUR="):
            try:
                _compile_after_hour = int(line.split("=", 1)[1].strip())
            except ValueError:
                pass
            break
...
def append_to_daily_log(content: str, section: str = "Session") -> None:
    """Append content to today's daily log with project tag."""
